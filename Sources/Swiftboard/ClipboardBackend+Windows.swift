    ...
    struct WindowsClipboardBackend: ClipboardBackend {
        ...
        func read(maxBytes: Int, syncImages: Bool) -> ClipboardItem? {
            guard self.openClipboardWithRetry() else {
        ...
        func write(_ item: ClipboardItem) {
            ...
            switch item.kind {
            case .text:
                self.writeUnicodeText(item.text ?? "")
            case .image:
                if let png = item.imagePNG {
                    self.writeImagePNG(png)
                }
            }
        }
        ...
        private func dibToBMP(_ dib: [UInt8]) -> [UInt8]? {
            ...
            let paletteEntries: Int = if clrUsed != 0 {
                clrUsed
            } else if bpp <= 8 {
                1 << bpp
            } else {
            ...
            self.appendLE32(&header, UInt32(fileSize))
            self.appendLE16(&header, 0)
            self.appendLE16(&header, 0)
            self.appendLE32(&header, UInt32(offBits))
            return header + dib
        }
        ...
        private func writeImagePNG(_ png: Data) {
            ...
            let w = Int(width)
            let h = Int(height)
            let dib = self.buildBGRADIB(rgba: rgba, width: w, height: h)
        ...
        private func buildBGRADIB(rgba: UnsafeMutablePointer<UInt8>, width: Int, height: Int) -> [UInt8] {
            ...
            self.appendLE32(&dib, 40) // biSize
            self.appendLE32(&dib, UInt32(width)) // biWidth
            self.appendLE32(&dib, UInt32(height)) // biHeight (positive => bottom-up)
            self.appendLE16(&dib, 1) // biPlanes
            self.appendLE16(&dib, 32) // biBitCount
            self.appendLE32(&dib, 0) // biCompression = BI_RGB
            self.appendLE32(&dib, UInt32(imageSize)) // biSizeImage
            self.appendLE32(&dib, 0) // biXPelsPerMeter
            self.appendLE32(&dib, 0) // biYPelsPerMeter
            self.appendLE32(&dib, 0) // biClrUsed
            self.appendLE32(&dib, 0) // biClrImportant
